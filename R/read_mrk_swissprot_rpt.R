mrk_swissprot_colnames <- function() {
    c(
      "marker_id",
      "marker_symbol",
      "marker_status",
...
mrk_swissprot_coltypes <- function() {
  "ccccccc"
}

mrk_swissprot_col_order <- function() {
  c(
    "marker_status",
    "marker_id",
    "marker_symbol",
...
read_mrk_swissprot_rpt <- function(file, n_max = Inf) {
  ...
  read_tsv(
    file = file,
    col_names = mrk_swissprot_colnames(),
    col_types = mrk_swissprot_coltypes(),
    n_max = n_max
  ) |>
    dplyr::mutate(
      ...
      swiss_prt_id = col_swiss_prt_id(.data$swiss_prt_id, sep = " ")
    ) |>
    dplyr::relocate(dplyr::all_of(mrk_swissprot_col_order()))
}
